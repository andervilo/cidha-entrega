...
import org.cidha.repository.ConflitoRepository;
import org.cidha.service.ConflitoQueryService;
import org.cidha.service.ConflitoService;
import org.cidha.service.criteria.ConflitoCriteria;
...
/**
 * REST controller for managing {@link org.cidha.domain.Conflito}.
 */
@RestController
@RequestMapping("/api")
public class ConflitoResource {
    ...
    public ResponseEntity<Conflito> updateConflito(
        ...
        Conflito result = conflitoService.update(conflito);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, conflito.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/conflitos/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Conflito> partialUpdateConflito(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, conflito.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<Conflito>> getAllConflitos(
        ConflitoCriteria criteria,
