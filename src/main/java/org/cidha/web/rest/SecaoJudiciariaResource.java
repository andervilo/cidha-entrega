...
import org.cidha.repository.SecaoJudiciariaRepository;
...
/**
 * REST controller for managing {@link org.cidha.domain.SecaoJudiciaria}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class SecaoJudiciariaResource {
    ...
    public ResponseEntity<SecaoJudiciaria> updateSecaoJudiciaria(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, secaoJudiciaria.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/secao-judiciarias/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<SecaoJudiciaria> partialUpdateSecaoJudiciaria(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, secaoJudiciaria.getId().toString())
        );
    }
    ...
    public List<SecaoJudiciaria> getAllSecaoJudiciarias(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all SecaoJudiciarias");
        return secaoJudiciariaRepository.findAllWithEagerRelationships();
    }
    ...
    public ResponseEntity<SecaoJudiciaria> getSecaoJudiciaria(@PathVariable Long id) {
        ...
        Optional<SecaoJudiciaria> secaoJudiciaria = secaoJudiciariaRepository.findOneWithEagerRelationships(id);
