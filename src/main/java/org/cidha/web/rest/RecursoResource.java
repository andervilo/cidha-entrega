...
import org.cidha.service.RecursoQueryService;
...
/**
 * REST controller for managing {@link org.cidha.domain.Recurso}.
 */
@RestController
@RequestMapping("/api")
public class RecursoResource {
    ...
    public RecursoResource(RecursoService recursoService, RecursoRepository recursoRepository, RecursoQueryService recursoQueryService) {
    ...
    public ResponseEntity<Recurso> createRecurso(@RequestBody Recurso recurso) throws URISyntaxException {
        ...
        Recurso result = recursoService.save(recurso);
    ...
    public ResponseEntity<Recurso> updateRecurso(@PathVariable(value = "id", required = false) final Long id, @RequestBody Recurso recurso)
        ...
        Recurso result = recursoService.update(recurso);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, recurso.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Recurso> partialUpdateRecurso(
        ...
        Optional<Recurso> result = recursoService.partialUpdate(recurso);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, recurso.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<Recurso>> getAllRecursos(
        RecursoCriteria criteria,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get Recursos by criteria: {}", criteria);
        Page<Recurso> page = recursoQueryService.findByCriteria(criteria, pageable);
    ...
    public ResponseEntity<Recurso> getRecurso(@PathVariable Long id) {
        ...
        Optional<Recurso> recurso = recursoService.findOne(id);
