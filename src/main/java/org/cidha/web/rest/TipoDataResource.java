...
import org.cidha.service.TipoDataQueryService;
import org.cidha.service.TipoDataService;
...
/**
 * REST controller for managing {@link org.cidha.domain.TipoData}.
 */
@RestController
@RequestMapping("/api")
public class TipoDataResource {
    ...
    public ResponseEntity<TipoData> updateTipoData(
        ...
        TipoData result = tipoDataService.update(tipoData);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, tipoData.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/tipo-data/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<TipoData> partialUpdateTipoData(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, tipoData.getId().toString())
        );
    }
