...
import org.cidha.service.ConcessaoLiminarCassadaQueryService;
import org.cidha.service.ConcessaoLiminarCassadaService;
import org.cidha.service.criteria.ConcessaoLiminarCassadaCriteria;
...
/**
 * REST controller for managing {@link org.cidha.domain.ConcessaoLiminarCassada}.
 */
@RestController
@RequestMapping("/api")
public class ConcessaoLiminarCassadaResource {
    ...
    public ResponseEntity<ConcessaoLiminarCassada> updateConcessaoLiminarCassada(
        ...
        ConcessaoLiminarCassada result = concessaoLiminarCassadaService.update(concessaoLiminarCassada);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, concessaoLiminarCassada.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/concessao-liminar-cassadas/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<ConcessaoLiminarCassada> partialUpdateConcessaoLiminarCassada(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, concessaoLiminarCassada.getId().toString())
        );
    }
