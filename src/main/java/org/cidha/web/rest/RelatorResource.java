...
import org.cidha.repository.RelatorRepository;
import org.cidha.service.RelatorQueryService;
import org.cidha.service.RelatorService;
import org.cidha.service.criteria.RelatorCriteria;
...
/**
 * REST controller for managing {@link org.cidha.domain.Relator}.
 */
@RestController
@RequestMapping("/api")
public class RelatorResource {
    ...
    public ResponseEntity<Relator> updateRelator(@PathVariable(value = "id", required = false) final Long id, @RequestBody Relator relator)
        ...
        Relator result = relatorService.update(relator);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, relator.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/relators/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Relator> partialUpdateRelator(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, relator.getId().toString())
        );
    }
     ...
     */
    @GetMapping("/relators")
    public ResponseEntity<List<Relator>> getAllRelators(
        RelatorCriteria criteria,
