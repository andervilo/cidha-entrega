...
import org.cidha.repository.OpcaoRecursoRepository;
import org.cidha.service.OpcaoRecursoQueryService;
import org.cidha.service.OpcaoRecursoService;
import org.cidha.service.criteria.OpcaoRecursoCriteria;
...
/**
 * REST controller for managing {@link org.cidha.domain.OpcaoRecurso}.
 */
@RestController
@RequestMapping("/api")
public class OpcaoRecursoResource {
    ...
    public ResponseEntity<OpcaoRecurso> updateOpcaoRecurso(
        ...
        OpcaoRecurso result = opcaoRecursoService.update(opcaoRecurso);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, opcaoRecurso.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/opcao-recursos/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<OpcaoRecurso> partialUpdateOpcaoRecurso(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, opcaoRecurso.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<OpcaoRecurso>> getAllOpcaoRecursos(
        OpcaoRecursoCriteria criteria,
