...
import org.cidha.service.JurisprudenciaQueryService;
import org.cidha.service.JurisprudenciaService;
import org.cidha.service.criteria.JurisprudenciaCriteria;
...
/**
 * REST controller for managing {@link org.cidha.domain.Jurisprudencia}.
 */
@RestController
@RequestMapping("/api")
public class JurisprudenciaResource {
    ...
    public ResponseEntity<Jurisprudencia> updateJurisprudencia(
        ...
        Jurisprudencia result = jurisprudenciaService.update(jurisprudencia);
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, jurisprudencia.getId().toString()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/jurisprudencias/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Jurisprudencia> partialUpdateJurisprudencia(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, jurisprudencia.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<Jurisprudencia>> getAllJurisprudencias(
        JurisprudenciaCriteria criteria,
