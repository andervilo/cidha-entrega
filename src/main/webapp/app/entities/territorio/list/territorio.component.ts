...
export class TerritorioComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.territorioService
      ...
      .subscribe({
        next: (res: HttpResponse<ITerritorio[]>) => {
  ...
  trackId(_index: number, item: ITerritorio): number {
    return item.id!;
  }

  delete(territorio: ITerritorio): void {
    const modalRef = this.modalService.open(TerritorioDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.territorio = territorio;
  ...
  protected onSuccess(data: ITerritorio[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
