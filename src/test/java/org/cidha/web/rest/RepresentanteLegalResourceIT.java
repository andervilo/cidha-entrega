...
import org.cidha.repository.RepresentanteLegalRepository;
import org.cidha.service.RepresentanteLegalService;
import org.cidha.service.criteria.RepresentanteLegalCriteria;
...
class RepresentanteLegalResourceIT {
    ...
    @Mock
    private RepresentanteLegalRepository representanteLegalRepositoryMock;

    @Mock
    private RepresentanteLegalService representanteLegalServiceMock;
    ...
    void getAllRepresentanteLegalsWithEagerRelationshipsIsEnabled() throws Exception {
        when(representanteLegalServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRepresentanteLegalMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(representanteLegalServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllRepresentanteLegalsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(representanteLegalServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void getAllRepresentanteLegalsByTipoRepresentanteIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, TipoRepresentante.class).isEmpty()) {
            tipoRepresentante = TipoRepresentanteResourceIT.createEntity(em);
            em.persist(tipoRepresentante);
            em.flush();
        } else {
            tipoRepresentante = TestUtil.findAll(em, TipoRepresentante.class).get(0);
        }
        em.persist(tipoRepresentante);
        em.flush();
    ...
    void getAllRepresentanteLegalsByProcessoConflitoIsEqualToSomething() throws Exception {
        // Initialize the database
        representanteLegalRepository.saveAndFlush(representanteLegal);
        ParteInteresssada processoConflito;
        if (TestUtil.findAll(em, ParteInteresssada.class).isEmpty()) {
            processoConflito = ParteInteresssadaResourceIT.createEntity(em);
            em.persist(processoConflito);
            em.flush();
        } else {
            processoConflito = TestUtil.findAll(em, ParteInteresssada.class).get(0);
        }
        em.persist(processoConflito);
        em.flush();
