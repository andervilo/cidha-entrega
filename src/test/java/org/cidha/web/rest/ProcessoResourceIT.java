...
import static org.cidha.web.rest.TestUtil.sameNumber;
...
import org.cidha.domain.enumeration.StatusProcesso;
...
class ProcessoResourceIT {
    ...
    void getAllProcessosByConcessaoLiminarIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        ConcessaoLiminar concessaoLiminar;
        if (TestUtil.findAll(em, ConcessaoLiminar.class).isEmpty()) {
            concessaoLiminar = ConcessaoLiminarResourceIT.createEntity(em);
            em.persist(concessaoLiminar);
            em.flush();
        } else {
            concessaoLiminar = TestUtil.findAll(em, ConcessaoLiminar.class).get(0);
        }
        em.persist(concessaoLiminar);
        em.flush();
    ...
    void getAllProcessosByConcessaoLiminarCassadaIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        ConcessaoLiminarCassada concessaoLiminarCassada;
        if (TestUtil.findAll(em, ConcessaoLiminarCassada.class).isEmpty()) {
            concessaoLiminarCassada = ConcessaoLiminarCassadaResourceIT.createEntity(em);
            em.persist(concessaoLiminarCassada);
            em.flush();
        } else {
            concessaoLiminarCassada = TestUtil.findAll(em, ConcessaoLiminarCassada.class).get(0);
        }
        em.persist(concessaoLiminarCassada);
        em.flush();
    ...
    void getAllProcessosByEmbargoDeclaracaoIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        EmbargoDeclaracao embargoDeclaracao;
        if (TestUtil.findAll(em, EmbargoDeclaracao.class).isEmpty()) {
            embargoDeclaracao = EmbargoDeclaracaoResourceIT.createEntity(em);
            em.persist(embargoDeclaracao);
            em.flush();
        } else {
            embargoDeclaracao = TestUtil.findAll(em, EmbargoDeclaracao.class).get(0);
        }
        em.persist(embargoDeclaracao);
        em.flush();
    ...
    void getAllProcessosByEmbargoDeclaracaoAgravoIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        EmbargoDeclaracaoAgravo embargoDeclaracaoAgravo;
        if (TestUtil.findAll(em, EmbargoDeclaracaoAgravo.class).isEmpty()) {
            embargoDeclaracaoAgravo = EmbargoDeclaracaoAgravoResourceIT.createEntity(em);
            em.persist(embargoDeclaracaoAgravo);
            em.flush();
        } else {
            embargoDeclaracaoAgravo = TestUtil.findAll(em, EmbargoDeclaracaoAgravo.class).get(0);
        }
        em.persist(embargoDeclaracaoAgravo);
        em.flush();
    ...
    void getAllProcessosByEmbargoRecursoEspecialIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        EmbargoRecursoEspecial embargoRecursoEspecial;
        if (TestUtil.findAll(em, EmbargoRecursoEspecial.class).isEmpty()) {
            embargoRecursoEspecial = EmbargoRecursoEspecialResourceIT.createEntity(em);
            em.persist(embargoRecursoEspecial);
            em.flush();
        } else {
            embargoRecursoEspecial = TestUtil.findAll(em, EmbargoRecursoEspecial.class).get(0);
        }
        em.persist(embargoRecursoEspecial);
        em.flush();
    ...
    void getAllProcessosByEmbargoRespReIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        EmbargoRespRe embargoRespRe;
        if (TestUtil.findAll(em, EmbargoRespRe.class).isEmpty()) {
            embargoRespRe = EmbargoRespReResourceIT.createEntity(em);
            em.persist(embargoRespRe);
            em.flush();
        } else {
            embargoRespRe = TestUtil.findAll(em, EmbargoRespRe.class).get(0);
        }
        em.persist(embargoRespRe);
        em.flush();
    ...
    void getAllProcessosByTipoDecisaoIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, TipoDecisao.class).isEmpty()) {
            tipoDecisao = TipoDecisaoResourceIT.createEntity(em);
            em.persist(tipoDecisao);
            em.flush();
        } else {
            tipoDecisao = TestUtil.findAll(em, TipoDecisao.class).get(0);
        }
        em.persist(tipoDecisao);
        em.flush();
    ...
    void getAllProcessosByTipoEmpreendimentoIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, TipoEmpreendimento.class).isEmpty()) {
            tipoEmpreendimento = TipoEmpreendimentoResourceIT.createEntity(em);
            em.persist(tipoEmpreendimento);
            em.flush();
        } else {
            tipoEmpreendimento = TestUtil.findAll(em, TipoEmpreendimento.class).get(0);
        }
        em.persist(tipoEmpreendimento);
        em.flush();
    ...
    void getAllProcessosBySecaoJudiciariaIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, SecaoJudiciaria.class).isEmpty()) {
            secaoJudiciaria = SecaoJudiciariaResourceIT.createEntity(em);
            em.persist(secaoJudiciaria);
            em.flush();
        } else {
            secaoJudiciaria = TestUtil.findAll(em, SecaoJudiciaria.class).get(0);
        }
        em.persist(secaoJudiciaria);
        em.flush();
    ...
    void getAllProcessosByComarcaIsEqualToSomething() throws Exception {
        ...
        processoRepository.saveAndFlush(processo);
        Comarca comarca;
        if (TestUtil.findAll(em, Comarca.class).isEmpty()) {
            comarca = ComarcaResourceIT.createEntity(em);
            em.persist(comarca);
            em.flush();
        } else {
            comarca = TestUtil.findAll(em, Comarca.class).get(0);
        }
        em.persist(comarca);
        em.flush();
    ...
    void getAllProcessosByTerritorioIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, Territorio.class).isEmpty()) {
            territorio = TerritorioResourceIT.createEntity(em);
            em.persist(territorio);
            em.flush();
        } else {
            territorio = TestUtil.findAll(em, Territorio.class).get(0);
        }
        em.persist(territorio);
        em.flush();
    ...
    void getAllProcessosByAtividadeExploracaoIlegalIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        AtividadeExploracaoIlegal atividadeExploracaoIlegal;
        if (TestUtil.findAll(em, AtividadeExploracaoIlegal.class).isEmpty()) {
            atividadeExploracaoIlegal = AtividadeExploracaoIlegalResourceIT.createEntity(em);
            em.persist(atividadeExploracaoIlegal);
            em.flush();
        } else {
            atividadeExploracaoIlegal = TestUtil.findAll(em, AtividadeExploracaoIlegal.class).get(0);
        }
        em.persist(atividadeExploracaoIlegal);
        em.flush();
    ...
    void getAllProcessosByUnidadeConservacaoIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, UnidadeConservacao.class).isEmpty()) {
            unidadeConservacao = UnidadeConservacaoResourceIT.createEntity(em);
            em.persist(unidadeConservacao);
            em.flush();
        } else {
            unidadeConservacao = TestUtil.findAll(em, UnidadeConservacao.class).get(0);
        }
        em.persist(unidadeConservacao);
        em.flush();
    ...
    void getAllProcessosByEnvolvidosConflitoLitigioIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        EnvolvidosConflitoLitigio envolvidosConflitoLitigio;
        if (TestUtil.findAll(em, EnvolvidosConflitoLitigio.class).isEmpty()) {
            envolvidosConflitoLitigio = EnvolvidosConflitoLitigioResourceIT.createEntity(em);
            em.persist(envolvidosConflitoLitigio);
            em.flush();
        } else {
            envolvidosConflitoLitigio = TestUtil.findAll(em, EnvolvidosConflitoLitigio.class).get(0);
        }
        em.persist(envolvidosConflitoLitigio);
        em.flush();
    ...
    void getAllProcessosByTerraIndigenaIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, TerraIndigena.class).isEmpty()) {
            terraIndigena = TerraIndigenaResourceIT.createEntity(em);
            em.persist(terraIndigena);
            em.flush();
        } else {
            terraIndigena = TestUtil.findAll(em, TerraIndigena.class).get(0);
        }
        em.persist(terraIndigena);
        em.flush();
    ...
    void getAllProcessosByProcessoConflitoIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        ProcessoConflito processoConflito;
        if (TestUtil.findAll(em, ProcessoConflito.class).isEmpty()) {
            processoConflito = ProcessoConflitoResourceIT.createEntity(em);
            em.persist(processoConflito);
            em.flush();
        } else {
            processoConflito = TestUtil.findAll(em, ProcessoConflito.class).get(0);
        }
        em.persist(processoConflito);
        em.flush();
    ...
    void getAllProcessosByParteInteresssadaIsEqualToSomething() throws Exception {
        // Initialize the database
        processoRepository.saveAndFlush(processo);
        ParteInteresssada parteInteresssada;
        if (TestUtil.findAll(em, ParteInteresssada.class).isEmpty()) {
            parteInteresssada = ParteInteresssadaResourceIT.createEntity(em);
            em.persist(parteInteresssada);
            em.flush();
        } else {
            parteInteresssada = TestUtil.findAll(em, ParteInteresssada.class).get(0);
        }
        em.persist(parteInteresssada);
        em.flush();
    ...
    void getAllProcessosByRelatorIsEqualToSomething() throws Exception {
        ...
        processoRepository.saveAndFlush(processo);
        Relator relator;
        if (TestUtil.findAll(em, Relator.class).isEmpty()) {
            relator = RelatorResourceIT.createEntity(em);
            em.persist(relator);
            em.flush();
        } else {
            relator = TestUtil.findAll(em, Relator.class).get(0);
        }
        em.persist(relator);
        em.flush();
    ...
    void getAllProcessosByQuilomboIsEqualToSomething() throws Exception {
        ...
        processoRepository.saveAndFlush(processo);
        Quilombo quilombo;
        if (TestUtil.findAll(em, Quilombo.class).isEmpty()) {
            quilombo = QuilomboResourceIT.createEntity(em);
            em.persist(quilombo);
            em.flush();
        } else {
            quilombo = TestUtil.findAll(em, Quilombo.class).get(0);
        }
        em.persist(quilombo);
        em.flush();
    ...
    void getAllProcessosByProblemaJuridicoIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, ProblemaJuridico.class).isEmpty()) {
            problemaJuridico = ProblemaJuridicoResourceIT.createEntity(em);
            em.persist(problemaJuridico);
            em.flush();
        } else {
            problemaJuridico = TestUtil.findAll(em, ProblemaJuridico.class).get(0);
        }
        em.persist(problemaJuridico);
        em.flush();
