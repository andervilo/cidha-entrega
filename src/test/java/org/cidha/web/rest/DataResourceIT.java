...
class DataResourceIT {
    ...
    void getAllDataWithEagerRelationshipsIsEnabled() throws Exception {
        when(dataServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restDataMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(dataServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllDataWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(dataServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void getAllDataByTipoDataIsEqualToSomething() throws Exception {
        ...
        dataRepository.saveAndFlush(data);
        TipoData tipoData;
        if (TestUtil.findAll(em, TipoData.class).isEmpty()) {
            tipoData = TipoDataResourceIT.createEntity(em);
            em.persist(tipoData);
            em.flush();
        } else {
            tipoData = TestUtil.findAll(em, TipoData.class).get(0);
        }
        em.persist(tipoData);
        em.flush();
