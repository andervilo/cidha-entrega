...
class SecaoJudiciariaResourceIT {
    ...
    @Mock
    private SecaoJudiciariaRepository secaoJudiciariaRepositoryMock;
    ...
    void getAllSecaoJudiciariasWithEagerRelationshipsIsEnabled() throws Exception {
        when(secaoJudiciariaRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSecaoJudiciariaMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(secaoJudiciariaRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllSecaoJudiciariasWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(secaoJudiciariaRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void partialUpdateSecaoJudiciariaWithPatch() throws Exception {
        ...
        partialUpdatedSecaoJudiciaria.sigla(UPDATED_SIGLA);
