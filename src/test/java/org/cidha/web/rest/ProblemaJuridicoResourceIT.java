...
class ProblemaJuridicoResourceIT {
    ...
    void getAllProblemaJuridicosByFundamentacaoDoutrinariaIsEqualToSomething() throws Exception {
        // Initialize the database
        problemaJuridicoRepository.saveAndFlush(problemaJuridico);
        FundamentacaoDoutrinaria fundamentacaoDoutrinaria;
        if (TestUtil.findAll(em, FundamentacaoDoutrinaria.class).isEmpty()) {
            fundamentacaoDoutrinaria = FundamentacaoDoutrinariaResourceIT.createEntity(em);
            em.persist(fundamentacaoDoutrinaria);
            em.flush();
        } else {
            fundamentacaoDoutrinaria = TestUtil.findAll(em, FundamentacaoDoutrinaria.class).get(0);
        }
        em.persist(fundamentacaoDoutrinaria);
        em.flush();
    ...
    void getAllProblemaJuridicosByJurisprudenciaIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, Jurisprudencia.class).isEmpty()) {
            jurisprudencia = JurisprudenciaResourceIT.createEntity(em);
            em.persist(jurisprudencia);
            em.flush();
        } else {
            jurisprudencia = TestUtil.findAll(em, Jurisprudencia.class).get(0);
        }
        em.persist(jurisprudencia);
        em.flush();
    ...
    void getAllProblemaJuridicosByFundamentacaoLegalIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, FundamentacaoLegal.class).isEmpty()) {
            fundamentacaoLegal = FundamentacaoLegalResourceIT.createEntity(em);
            em.persist(fundamentacaoLegal);
            em.flush();
        } else {
            fundamentacaoLegal = TestUtil.findAll(em, FundamentacaoLegal.class).get(0);
        }
        em.persist(fundamentacaoLegal);
        em.flush();
    ...
    void getAllProblemaJuridicosByInstrumentoInternacionalIsEqualToSomething() throws Exception {
        // Initialize the database
        problemaJuridicoRepository.saveAndFlush(problemaJuridico);
        InstrumentoInternacional instrumentoInternacional;
        if (TestUtil.findAll(em, InstrumentoInternacional.class).isEmpty()) {
            instrumentoInternacional = InstrumentoInternacionalResourceIT.createEntity(em);
            em.persist(instrumentoInternacional);
            em.flush();
        } else {
            instrumentoInternacional = TestUtil.findAll(em, InstrumentoInternacional.class).get(0);
        }
        em.persist(instrumentoInternacional);
        em.flush();
    ...
    void partialUpdateProblemaJuridicoWithPatch() throws Exception {
        ...
        partialUpdatedProblemaJuridico.prolemaJuridicoRespondido(UPDATED_PROLEMA_JURIDICO_RESPONDIDO);
