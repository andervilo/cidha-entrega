...
class ProcessoConflitoResourceIT {
    ...
    void getAllProcessoConflitosByConflitoIsEqualToSomething() throws Exception {
        ...
        processoConflitoRepository.saveAndFlush(processoConflito);
        Conflito conflito;
        if (TestUtil.findAll(em, Conflito.class).isEmpty()) {
            conflito = ConflitoResourceIT.createEntity(em);
            em.persist(conflito);
            em.flush();
        } else {
            conflito = TestUtil.findAll(em, Conflito.class).get(0);
        }
        em.persist(conflito);
        em.flush();
    ...
    void getAllProcessoConflitosByDireitoIsEqualToSomething() throws Exception {
        ...
        processoConflitoRepository.saveAndFlush(processoConflito);
        Direito direito;
        if (TestUtil.findAll(em, Direito.class).isEmpty()) {
            direito = DireitoResourceIT.createEntity(em);
            em.persist(direito);
            em.flush();
        } else {
            direito = TestUtil.findAll(em, Direito.class).get(0);
        }
        em.persist(direito);
        em.flush();
