...
import org.cidha.repository.RecursoRepository;
import org.cidha.service.RecursoService;
import org.cidha.service.criteria.RecursoCriteria;
...
class RecursoResourceIT {
    ...
    @Mock
    private RecursoService recursoServiceMock;
    ...
    void getAllRecursosWithEagerRelationshipsIsEnabled() throws Exception {
        when(recursoServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restRecursoMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(recursoServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllRecursosWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(recursoServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void getAllRecursosByTipoRecursoIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, TipoRecurso.class).isEmpty()) {
            tipoRecurso = TipoRecursoResourceIT.createEntity(em);
            em.persist(tipoRecurso);
            em.flush();
        } else {
            tipoRecurso = TestUtil.findAll(em, TipoRecurso.class).get(0);
        }
        em.persist(tipoRecurso);
        em.flush();
    ...
    void getAllRecursosByOpcaoRecursoIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, OpcaoRecurso.class).isEmpty()) {
            opcaoRecurso = OpcaoRecursoResourceIT.createEntity(em);
            em.persist(opcaoRecurso);
            em.flush();
        } else {
            opcaoRecurso = TestUtil.findAll(em, OpcaoRecurso.class).get(0);
        }
        em.persist(opcaoRecurso);
        em.flush();
