...
class ParteInteresssadaResourceIT {
    ...
    void getAllParteInteresssadasByRepresentanteLegalIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, RepresentanteLegal.class).isEmpty()) {
            representanteLegal = RepresentanteLegalResourceIT.createEntity(em);
            em.persist(representanteLegal);
            em.flush();
        } else {
            representanteLegal = TestUtil.findAll(em, RepresentanteLegal.class).get(0);
        }
        em.persist(representanteLegal);
        em.flush();
    ...
    void partialUpdateParteInteresssadaWithPatch() throws Exception {
        ...
        partialUpdatedParteInteresssada.classificacao(UPDATED_CLASSIFICACAO);
